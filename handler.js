...
var pushname = this.getName(id)   
const groupName =  groupMetadata.subject || []
const groupLength = groupMetadata.participants.length
let wel = `https://api.caliph.biz.id/api/welcome2?username=${pushname}&groupname=${groupName}&membercount=${groupLength}&profile=${pp}&background=https://telegra.ph/file/8f5ea9bcc4ec4e5868a8b.jpg&apikey=caliphkey`
let lea = `https://api.caliph.biz.id/api/goodbye2?username=${pushname}&groupname=${groupName}&membercount=${groupLength}&profile=${pp}&background=https://telegra.ph/file/8f5ea9bcc4ec4e5868a8b.jpg&apikey=caliphkey`          
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? wel : lea)).buffer(), text, `BY R-BOT`, action == 'add' ? 'Wlcm' : 'By', action === 'add' ? '.intro' : 'hehe🗿',
     )*/
...
var ppgc = "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
  ...
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? pp : pp)).buffer(), text, `BY R-BOT`, action == 'add' ? 'Wlcm' : 'By', action === 'add' ? '.intro' : 'hehe🗿',
     )
